...
// Controlador para obtener todas la tareas de la Base de Datos.
...
// Controlador para crear nueva tarea en la Base de Datos.
...
try {
    ...
    const id= req.params.id
    const {_id}= req.user  //ID DEL USUARIO QUE SE LOGEO
    ...
    if({_id} != taskAeliminar.idUser){   //conbierte mi objeto a string para comparar los valores
        return res.status(400).json({
            message: 'No tenes permiso para eliminar la tarea de otro usuario'
        })
    }
