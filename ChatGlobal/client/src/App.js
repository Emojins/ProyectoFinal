...
function App() {
  ...
  const handlerSubmit = (e) => {
    ...
    if(nickname !== ''){
      ...
      socket.emit('message', message, `${nickname} (${select})`)
              ...
              <div className="card-body">
                ...
                <form onSubmit={nicknameSubmit}>
                  <div className="d-flex mb-3">
                    ...
                    <button className="btn btn-dark mx-3" type="submit" id="btn-nickname" disabled={disabled}>Establecer</button>

                    <select className="btn btn-light" name='select' onChange={e => setSelect(e.target.value)}>
                    <option value="null" selected>Seleccione un rol</option>
                    <option value="usuario">Usuario</option>
                    <option value="compositor">Artista</option>
                    <option value="escritor">Productor</option>
                  </select>
                  <button className="btn btn-dark mx-3" onClick={()=>alert(select)}>Establecer</button>
                  </div>
              ...
              <div className="card-body">
                ...
                {storedMessages.map((message, index) => (
                  <div key={index} className={`d-flex p-3 ${message.from === nickname ? "justify-content-end" : "justify-content-start"}`}>
                    <div className={`card mb-6 shadow border-1 ${message.from === nickname ? "bg-success bg-opacity-25" : "bg-light"}`}>
                      <div className="card-body">
